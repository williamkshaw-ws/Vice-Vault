...
export default function TrophyCase({ uniqueBalls }: TrophyCaseProps) {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(rackRef.current, { 
        cacheBust: true, 
        pixelRatio: 2,
        skipFonts: true, // Safari strictly blocks font loading inside canvas
        filter: (node) => {
          // Safari's canvas renderer crashes if it encounters SVG noise filters
...
with open(path, 'w') as f:
    f.write(content)
print("Rewrote TrophyCase.tsx with html-to-image fixes")
