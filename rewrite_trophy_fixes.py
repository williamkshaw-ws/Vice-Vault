...
export default function TrophyCase({ uniqueBalls }: TrophyCaseProps) {
  ...
  return (
    <div className="relative group/rack z-0">
      {/* Save Button (shows on hover) */}
      <button 
        onClick={handleSaveImage}
        disabled={isSaving || uniqueBalls.length === 0}
        className="absolute top-4 right-4 z-[60] bg-neutral-900/80 hover:bg-[#2563eb] text-white p-2 rounded-lg backdrop-blur-sm border border-white/10 shadow-xl opacity-0 group-hover/rack:opacity-100 transition-all duration-300 disabled:opacity-50 flex items-center gap-2"
        title="Save Image"
      >
            ...
            return (
              <div key={item.id} className="relative flex flex-col items-center group">
                ...
                <div className="relative w-12 h-12 sm:w-14 sm:h-14">
                  ...
                  <div className="absolute inset-0 bg-black/90 blur-sm rounded-full translate-y-1"></div>
